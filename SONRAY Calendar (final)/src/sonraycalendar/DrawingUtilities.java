...
public final class DrawingUtilities { //utility class for drawing stuff
	//chose to draw images then return smaller versions of those images
	//instead of just returning path2d (even though paths would've taken
	//up a lot less lines of codes) because it looked bad quality when
	//drawn, even when set rendering hints
	private DrawingUtilities() {}
	
	private static Image getUnscaledIcon(Path2D path, Color bgcolor, Color strcolor, int width, int height, int strwidth) {
		...
		g2d.setStroke(new BasicStroke(strwidth, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
		g2d.draw(path);
	...
	public static String getFittedString(FontMetrics fontMetrics, String str, int space) {
		int len = fontMetrics.stringWidth(str);
		int oldstrlen = str.length();
		int newstrlen = oldstrlen;
		while (len > space - 10) len = fontMetrics.stringWidth(str.substring(0, (--newstrlen)) + "...");
	...
	public static Image clock(int size, Color bgcolor, Color strcolor) {
		...
		clock.append(new Ellipse2D.Float(strwidth / 2, strwidth / 2, unscaledSize - strwidth, unscaledSize - strwidth), false);
		clock.append(new Ellipse2D.Float(space + strwidth, space + strwidth, radius * 2, radius * 2), false);
		
		clock.moveTo(centre, centre - radius/2);
		clock.lineTo(centre, centre);
		...
		for (double i = 0; i <= Math.PI / 2; i += Math.PI / 6) {
			...
			int x2 = (int) ((radius - (space + strwidth/2)) * Math.cos(i));
			int y2 = (int) ((radius - (space + strwidth/2)) * Math.sin(i));
		...
		Image unscaledClock = getUnscaledIcon(clock, bgcolor, strcolor, unscaledSize, unscaledSize, strwidth);
		
		return unscaledClock.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
	...
	public static Image notes(int width, Color bgcolor, Color strcolor) {
		...
		Image unscaledNotes = getUnscaledIcon(notes, bgcolor, strcolor, unscaledWidth, unscaledHeight, 2 * halfstrwidth);
		
		return unscaledNotes.getScaledInstance(width, width * 9 / 16, Image.SCALE_AREA_AVERAGING);
	...
	public static Image pencil(int size, Color bgcolor, Color strcolor) {
		...
		for (int i = 1; i < 4; i++) {
			pencil.moveTo(bottomLeftX + (bottomRightX - bottomLeftX) * i / 3, bottomLeftY + (bottomRightX - bottomLeftX) * i / 3);
			pencil.lineTo(topLeftX - spacing + (topRightX - topLeftX) * i / 3, topLeftY + spacing + (topRightY - topLeftY) * i / 3);
		}
		
		Image unscaledPencil = getUnscaledIcon(pencil, bgcolor, strcolor, unscaledSize, unscaledSize, 2 * halfstrwidth);
		
		return unscaledPencil.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
	...
	public static Image search(int size, Color bgcolor, Color strcolor) {
		...
		search.append(new Ellipse2D.Float(halfstrwidth, halfstrwidth, 2 * radius, 2 * radius), false);
		search.append(new Ellipse2D.Float(halfstrwidth + space, halfstrwidth + space, 2 * (radius - space), 2 * (radius - space)), false);
		...
		Image unscaledSearch = getUnscaledIcon(search, bgcolor, strcolor, unscaledSize, unscaledSize, 2 * halfstrwidth);
		
		return unscaledSearch.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
	...
	public static Image plus(int size, Color bgcolor, Color strcolor) {
		...
		Image unscaledAdd = getUnscaledIcon(plus, bgcolor, strcolor, unscaledSize, unscaledSize, 2 * halfstrwidth);
		
		return unscaledAdd.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
	...
	public static Image leftArrow(int width, Color bgcolor, Color strcolor) {
		...
		leftArrow.lineTo(unscaledWidth - halfstrwidth, unscaledHeight - halfstrwidth);
		
		Image unscaledLeftArrow = getUnscaledIcon(leftArrow, bgcolor, strcolor, unscaledWidth, unscaledHeight, 2 * halfstrwidth);
		
		return unscaledLeftArrow.getScaledInstance(width, width * 16 / 9, Image.SCALE_AREA_AVERAGING);
	}
	...
	public static Image rightArrow(int width, Color bgcolor, Color strcolor) {
		...
		Image unscaledLeftArrow = getUnscaledIcon(rightArrow, bgcolor, strcolor, unscaledWidth, unscaledHeight, 2 * halfstrwidth);
	...
	public static Image garbage(int width, Color bgcolor, Color strcolor) {
		...
		Image unscaledAdd = getUnscaledIcon(garbage, bgcolor, strcolor, unscaledWidth, unscaledHeight, 2 * halfstrwidth);
		
		return unscaledAdd.getScaledInstance(width, width * 4 / 3, Image.SCALE_AREA_AVERAGING);
	}
	...
	public static Image check(int size, Color bgcolor, Color strcolor) {
		...
		check.append(new Ellipse2D.Float(halfstrwidth, halfstrwidth, unscaledSize - 2 * halfstrwidth, unscaledSize - 2 * halfstrwidth), false);
		...
		int currentx;
		check.lineTo(currentx = centre + x1 - space, lasty += (lastx - currentx) * -1 / slope);
		lastx = currentx;
		check.lineTo(currentx = centre - x2, lasty += (lastx - currentx) * slope);
		lastx = currentx;
		check.lineTo(currentx = centre - x1 + space, lasty += (lastx - currentx) * -1 / slope);
		check.closePath();
		
		Image unscaledAdd = getUnscaledIcon(check, bgcolor, strcolor, unscaledSize, unscaledSize, 2 * halfstrwidth);
		
		return unscaledAdd.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
	
	public static Image pin(int size, Color bgcolor, Color strcolor) {
		int unscaledSize = 4 * size + 64 - (4 * size) % 64;
		int halfstrwidth = unscaledSize / 16;
		
		Image location = new BufferedImage(unscaledSize, unscaledSize, BufferedImage.TYPE_INT_RGB);
		
		Graphics2D g2d = (Graphics2D) location.getGraphics();
		
		g2d.setColor(bgcolor);
		g2d.fillRect(0, 0, unscaledSize, unscaledSize);
		g2d.setColor(strcolor);
		g2d.setStroke(new BasicStroke(2 * halfstrwidth, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
		
		g2d.drawOval(unscaledSize / 4, halfstrwidth, unscaledSize / 2, unscaledSize / 2);
		g2d.drawArc(unscaledSize * 3 / 8, halfstrwidth + unscaledSize / 8, unscaledSize / 4, unscaledSize / 4, 90, 90);
		g2d.drawLine(unscaledSize / 2, unscaledSize / 2 + halfstrwidth, unscaledSize / 2, unscaledSize - halfstrwidth);
		
		return location.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
	
	public static Image ex(int size, Color bgcolor, Color strcolor) {
		int unscaledSize = 4 * size + 32 - (4 * size) % 32;
		int halfstrwidth = unscaledSize / 8;
		
		Path2D ex = new Path2D.Float();
		
		ex.moveTo(halfstrwidth, halfstrwidth);
		ex.lineTo(unscaledSize - halfstrwidth, unscaledSize - halfstrwidth);
		ex.moveTo(halfstrwidth, unscaledSize - halfstrwidth);
		ex.lineTo(unscaledSize - halfstrwidth, halfstrwidth);
		
		Image unscaledEx = getUnscaledIcon(ex, bgcolor, strcolor, unscaledSize, unscaledSize, 2 * halfstrwidth);
		
		return unscaledEx.getScaledInstance(size, size, Image.SCALE_AREA_AVERAGING);
	}
}