...
public class TopPanel extends JPanel {
	...
	private static class CurrentTimePanel extends JPanel { //panel to show current time and has button so can
		...
		private void init() {
			...
			now.setBounds(10, 17, 165, 20);
			add(now);
			
			goToCurrentMonth.setBounds(170, 20, 14, 14);
		...
		protected void paintComponent(Graphics g) {
			...
			g.drawImage(DrawingUtilities.pin(14, getBackground(), currentColor), 170, 20, null);
	...
	private void setSearchMode(boolean b) {
		...
		if (b) { //resizes panel and sets bounds of search field and exit search button
			...
			exitSearch.setBounds(getWidth() - 42, 10, 20, 20);
			searchField.setBounds(47, 7, getWidth() - 100, 27);
		} else { //resizes panel and sets bounds of every button (expect exit search) and of jlabel showing month
			leftArrow.setBounds(63, 7, 15, 27); // x = 0
			rightArrow.setBounds(90, 7, 15, 27); // x = 25
			...
			month.setSize(getFontMetrics(month.getFont()).stringWidth(month.getText()), 27);
			month.setLocation(getWidth() / 2 - month.getWidth() / 2 - 30, 7);
			add.setBounds(317, 7, 27, 27); // x = month.getWidth() + 80
