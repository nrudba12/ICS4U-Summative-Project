...
public final class Icon { //delete all deprecated once all methods fixed
	...
	@Deprecated
	public static Path2D pencil(double x, double y, double size) {
		double space = size / 8.0;
		
		double topLeftX = x + size * 3.0 / 4.0;
		double topLeftY = y;
		double topRightX = x + size;
		double topRightY = y + size / 4.0;
		
		double bottomRightX = x + size / 4.0 + space;
		double bottomRightY = y + size - space;
		double bottomLeftX = x + space;
		double bottomLeftY = y + size - (size / 4.0 + space);
		
		Path2D pencil = new Path2D.Double();
		
		pencil.moveTo(bottomRightX, bottomRightY);
		pencil.lineTo(x, y + size);
		pencil.lineTo(bottomLeftX, bottomLeftY);
		
		pencil.moveTo(bottomLeftX, bottomLeftY);
		pencil.lineTo(bottomRightX, bottomRightY);
		
		pencil.moveTo(topLeftX - space, topLeftY + space);
		pencil.lineTo(topRightX - space, topRightY + space);
		...
		for (int i = 1; i < 4; i++) {
			...
			pencil.lineTo(topLeftX - space + (topRightX - topLeftX) * i / 3,
					topLeftY + space + (topRightY - topLeftY) * i / 3);
		}
		
		return pencil;
	}
	@Deprecated
	public static Path2D check(double x, double y, double size) {
		double centrex = x + size / 2.0;
		double centrey = y + size / 2.0;
		double space = size / 16.0;
		double radius = size / 4.0;
		
		double x1 = radius * Math.cos(Math.PI / 12);
		double y1 = radius * Math.sin(Math.PI / 12);
		double x2 = radius * Math.cos(Math.PI * 5 / 12);
		double y2 = radius * Math.sin(Math.PI * 5 / 12);
		
		double slope = -(y2 - y1) / (x2 - x1);
		double lastx = centrex + x1;
		double lasty = centrey - space / 2.0 - y1;
		
		Path2D check = new Path2D.Double();
		
		check.append(new Ellipse2D.Double(x, y, size, size), false);
		
		check.moveTo(centrex - x1, centrey - space / 2.0 + y1);
		check.lineTo(centrex - x2, centrey - space / 2.0 + y2);
		check.lineTo(lastx, lasty);
		
		double currentx;
		check.lineTo(currentx = centrex + x1 - space, lasty
				+= (lastx - currentx) * -1 / slope);
		lastx = currentx;
		check.lineTo(currentx = centrex - x2, lasty
	...
	@Deprecated
	public static Path2D garbage(double x, double y, double width) {
		double height = width * 4 / 3;
		
		Path2D garbage = new Path2D.Float();
		
		garbage.moveTo(x, y);
		garbage.lineTo(x + width, y);
		garbage.lineTo(x + width, y + height);
		garbage.lineTo(x, y + height);
		garbage.closePath();
		
		double top = y + height * 5.0 / 32.0;
		
		garbage.moveTo(x, top);
		garbage.lineTo(x + width, top);
		
		for (int i = 1; i <= 3; i++) {
			double xpos = x + width * i / 4;
			
			garbage.moveTo(xpos, top + height / 8.0);
			garbage.lineTo(xpos, y + height - height / 8.0);
		}
	...
	public static Image location(int size, Color bgcolor, Color strcolor) {
		int unscaledSize = 4 * size + 64 - (4 * size) % 64;
		int halfstrwidth = unscaledSize * 3 / 64;
		
		Image location = new BufferedImage(unscaledSize, unscaledSize,
				BufferedImage.TYPE_INT_RGB);
		...
		g2d.setColor(strcolor);
		g2d.setStroke(new BasicStroke(2 * halfstrwidth, BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_ROUND));
		
		g2d.drawOval(unscaledSize / 4, halfstrwidth,
				unscaledSize / 2, unscaledSize / 2);
		g2d.drawArc(unscaledSize * 3 / 8, halfstrwidth + unscaledSize / 8, unscaledSize / 4,
				unscaledSize / 4, 90, 90);
		g2d.drawLine(unscaledSize / 2, unscaledSize / 2 + halfstrwidth,
				unscaledSize / 2, unscaledSize - halfstrwidth);
	...
	public static Image ex(int size, Color bgcolor, Color strcolor) {
		...
		Image unscaledEx = getUnscaledIcon(ex, bgcolor, strcolor,
				unscaledSize, unscaledSize, 2 * halfstrwidth);
		
		return unscaledEx.getScaledInstance(size, size,
				Image.SCALE_AREA_AVERAGING);
	}
}