...
public class CalendarPanel extends JPanel {
	...
	public void init() {
		...
		search.init(() -> setSearchMode(true));
	...
	private void setSearchMode(boolean b) {
		...
		if (b) {
			...
			CalendarDisplay display = Manager.getInstance().calendarDisplay;
			String text = Month.getName(display.getMonth()[0]) + " " + display.getMonth()[1];
			month.setText(text);
	...
	public void stopShowingTime() {
		for (Component c : Manager.getInstance().getContentPane().getComponents()) {
			if (c instanceof TimeDisplay) Manager.getInstance().remove(c);
		}
	}
	...
	private void drawSearchMode(Graphics2D g2d) {
		...
		g2d.drawImage(Icon.ex(20, bgcolor, excolor), exitSearch.getX(), 10, null);
	...
	private void drawRegular(Graphics2D g2d) {
		...
		g2d.drawImage(Icon.leftArrow(15, bgcolor, leftArrowColor), leftArrow.getX(), 7, null);
		Color rightArrowColor = rightArrow.hovering() ? Color.RED : Color.GRAY;
		g2d.drawImage(Icon.rightArrow(15, bgcolor, rightArrowColor), rightArrow.getX(), 7, null);
