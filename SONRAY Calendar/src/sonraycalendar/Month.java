...
public enum Month {
	...
	public static int getWeekday(int month, int day, int year) {		
		...
		int weekday = (day + (int) (2.6*renumMonth - 0.2) - 2*century
				+ yearInCentury + yearInCentury/4 + century/4) % 7;
		if (weekday < 0) weekday += 7;
		
		return weekday;
	}
	
	public static String getWeekdayAsString(int month, int day, int year) {
		return weekdays[getWeekday(month, day, year)];
	}
