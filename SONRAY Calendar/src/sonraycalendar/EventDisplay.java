...
public class EventDisplay extends JPanel {
	...
	private void initButtons() {
		...
		update.init(() -> {
			...
			manager.sidePanel.setDefaultEvents();
			int[] month = manager.calendarDisplay.getMonth();
			manager.calendarDisplay.setMonth(month[0], month[1]);
			manager.repaint();
		});
	...
	private void initTextFields() {
		...
		String strdate = event.getMonth() + " / " + event.getDay() + " / " + event.getYear();
	...
	private void drawRegularMode(Graphics2D g) {
		...
		g.drawImage(Icon.clock(26, Color.WHITE, Color.GRAY), 10, 60, null);
		g.drawImage(Icon.notes(26, Color.WHITE, Color.GRAY), 10, 100, null);
	...
	private void drawEditMode(Graphics2D g) {
		...
		g.drawImage(Icon.notes(20, Color.WHITE, Color.GRAY), 10, 135, null);
